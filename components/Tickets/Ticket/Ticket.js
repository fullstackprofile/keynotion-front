...
export const Ticket = ({price,sale,currency,items,type,other_type,attractive,id,event}) => {
    ...
    const onBook = () => {
        ...
        if (!context.session.itemsss?.length) {
            context.setSession(prev => ({itemsss: [...prev.itemsss,obj],count: 1}))
        }else{
  ...
  return (
    <div className={styles.ticket}>
        <div className={styles.body}>
            <div className={attractive ? styles.title_block_liner : styles.title_block}>
                <div className={styles.title_block_}>
               <p className={attractive ? styles.title_ :styles.title}>{type}</p>
               {other_type && <p className={attractive ? styles.subTitle_ : styles.subTitle}>{other_type}</p>}
                </div>
            </div>
            <div className={styles.content}>
                <div className={sale ? styles.content_price : styles.content_price_}>
                        ...
                        {sale ? 
                        <>
                        <p className={styles.price_sale}>{price}{currency}</p>
                        <p className={styles.price}>{sale}{currency}</p>
                        </> :
                ...
                <div className={styles.content_body}>
                    {
                        items.map(({title})=> <p className={styles.content_item}>{title}</p>)
                    }
                </div>
