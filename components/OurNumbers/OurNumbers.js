...
const numbers = [
  { text: 'CZ Phone Number:', number: '+420 228 887 628' },
      ...
      <div className={styles.ourNumbers_content}>
        <div className={styles.content_title_block}>
          <p className={styles.content_title}>
            Our <span className={styles.content_title_liner}>Numbers</span>
          </p>
        </div>
        <div className={styles.content_body}>
          {numbers.map(({ text, number }, index) => (
            <PhoneNumber key={index} text={text} number={number} />
          ))}
        </div>
      </div>
    </div>
